...
import (
	...
	"github.com/itimor/telegram-sender/cron"
...
func aconf() {
	...
	*conf = path.Join(runner.Cwd, "etc", "telegram-sender.local.yml")
	if file.IsExist(*conf) {
		return
	}

	*conf = path.Join(runner.Cwd, "etc", "telegram-sender.yml")
	if file.IsExist(*conf) {
		return
	}
	println(*conf)
